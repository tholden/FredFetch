function [vintdata] = vintall(series, varargin)
  ...
  vintdata = FredFetch.dispatch_(0, opt.parworkers, @FredFetch.vintrange, series, very_first, very_last, toPass{:});
  ...
  for s = 1:length(series)
    if ~isempty(vintdata(s).value)
      vintdata(s).publag = FredFetch.computePublag_(vintdata(s).date, vintdata(s).realtime, vintdata(s).value);
    else
